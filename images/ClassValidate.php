...
class ClassValidate{
    ...
    public function validateUserActive($email)
    {
        ...
        if($user["data"]["status"] == "confirmation"){
            if(strtotime($user["data"]["dataCriacao"])<= strtotime(date("Y-m-d H:i:s"))){
...
public function validateFinalAlt($arrVar)
{
    if(count($this->getErro())>0){
        $arrResponse=[
            "retorno"=>"erro",
        ...
        $this->cadastro->alteracao($arrVar);
    }
    return json_encode($arrResponse);
}
